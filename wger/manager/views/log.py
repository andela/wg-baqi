...
class WorkoutLogUpdateView(WgerFormMixin, UpdateView, LoginRequiredMixin):
    ...
    def get_context_data(self, **kwargs):
        context = super(WorkoutLogUpdateView,
...
def add(request, pk):
    ...
    # each
    # exercise: the entries for weight and repetitions have no indicator to
    # which
    ...
    if request.method == 'POST':
        ...
        # is
        # that the form expects a value for the exercise which is not present
        # in
        ...
        if dateform.is_valid() and session_form.is_valid() and\
                formset.is_valid():
            ...
            for instance in instances:
                ...
                instance.user = request.user
                instance.workout = day.training
                instance.session = WorkoutSession.objects.get(
                    user=request.user,
                    date=log_date)
                instance.date = log_date
                instance.save()
...
class WorkoutLogDetailView(DetailView, LoginRequiredMixin):
        ...
        for day_list in self.object.canonical_representation['day_list']:
            ...
            for set_list in day_list['set_list']:
                ...
                for exercise_list in set_list['exercise_list']:
                    ...
                    logs = exercise_list[
                        'obj'].workoutlog_set.filter(user=self.owner_user,
                                                     weight_unit__in=(1, 2),
                                                     workout=self.object) \
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        # Dispatch normally
        return super(WorkoutLogDetailView,
