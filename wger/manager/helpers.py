...
def render_workout_day(day, nr_of_weeks=7,
                       images=False, comments=False, only_table=False):
    '''
    ...
    data.append([_('Nr.'), _('Exercise'), _(
        'Reps')] + [_('Weight')] * nr_of_weeks)
        ...
        for exercise in set['exercise_list']:
            ...
            if exercise['has_weight']:
                ...
                for i in exercise['setting_text'].split(u'–'):
                    setting_out.append(Paragraph(i, styleSheet[
                        "Small"], bulletText=''))
            else:
                setting_out = Paragraph(exercise[
                    'setting_text'], styleSheet["Small"])
    ...
    for i in range(exercise_start, len(data) + 1):
        if not i % 2:
            table_style.append(('BACKGROUND', (1, i - 1),
                                (-1, i - 1), colors.lavender))
