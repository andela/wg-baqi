...
def view(request, id, slug=None):
    ...
    if not backgrounds:
        ...
        for muscle in exercise.muscles_secondary.all():
            if muscle.is_front:
                backgrounds_front.append('images/muscles/secondary/muscle'
                                         '-%s.svg' % muscle.id)
            else:
                backgrounds_back.append('images/muscles/secondary/muscle-%s'
                                        '.svg' % muscle.id)
...
class ExercisesEditAddView(WgerFormMixin):
    ...
    def get_form_class(self):
        ...
        # Define the exercise form here because only at this point during
        # the request have we access to the currently used language. In
        # other places Django defaults to 'en-us'.
...
class ExerciseDeleteView(WgerDeleteMixin,
    ...
    delete_message = ugettext_lazy('This will delete the exercise from all '
                                   'workouts.')
...
def decline(request, pk):
    ...
    exercise.save()
    messages.success(request, _('Exercise was '
