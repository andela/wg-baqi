...
class CopyWorkoutTestCase(WorkoutManagerTestCase):
    ...
    def copy_workout(self, owner=False):
        ...
        response = self.client.get(reverse(
            'manager:workout:copy', kwargs={'pk': '3'}))
        if not owner:
        ...
        response = self.client.get(reverse(
            'manager:workout:view', kwargs={'pk': 4}))
    ...
    def test_copy_shared_not_allowed(self):
        '''
        Test copying a workout from another shared user where user does not
        share workouts
        '''
    ...
    def test_copy_shared_allowed(self):
        '''
        Test copying a workout from another shared user where user does share
        workouts
        '''
        profile = UserProfile.objects.get(user__username='test')
