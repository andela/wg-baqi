...
class IcalToolsTestCase(WorkoutManagerTestCase):
    ...
    def test_next_weekday(self):
        ...
        # Find next monday
        self.assertEqual(next_weekday(start_date, 0), datetime.date(
            2013, 12, 9))

        # Find next wednesday
        self.assertEqual(next_weekday(start_date, 2), datetime.date(
            2013, 12, 11))

        # Find next saturday
        self.assertEqual(next_weekday(start_date, 5), datetime.date(
            2013, 12, 7))
...
class WorkoutICalExportTestCase(WorkoutManagerTestCase):
    ...
    def export_ical_token(self):
        '''
        Helper function that checks exporing an ical file using tokens for
        access
        '''
    ...
    def export_ical(self, fail=False):
        ...
        response = self.client.get(reverse(
            'manager:workout:ical', kwargs={'pk': 3}))
...
class ScheduleICalExportTestCase(WorkoutManagerTestCase):
    ...
    def export_ical(self, fail=False):
        ...
        response = self.client.get(reverse(
            'manager:schedule:ical', kwargs={'pk': 2}))
    ...
    def test_export_ical_other(self):
        '''
        Tests exporting a schedule as an ical file as a logged user not owning
        the data
        '''
