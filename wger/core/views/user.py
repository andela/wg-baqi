...
def registration(request):
    ...
    if request.user.is_authenticated and \
            not request.user.userprofile.is_temporary:
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if (request.user.has_perm('gym.manage_gym')
            or request.user.has_perm(
                'gym.gym_trainer')) and edit_user.userprofile.gym_id != \
...
class UserActivateView(LoginRequiredMixin,
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if (request.user.has_perm('gym.manage_gym') or request.user.has_perm(
            'gym.gym_trainer')) \
...
class UserDetailView(LoginRequiredMixin,
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if (user.has_perm('gym.manage_gym') or user.has_perm(
            'gym.gym_trainer')) \
                and not user.has_perm('gym.manage_gyms') \
                and user.userprofile.gym != self.get_object().userprofile.gym:
            return HttpResponseForbidden()
