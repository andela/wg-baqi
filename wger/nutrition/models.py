    ...
    def get_nutritional_values(self):
        ...
        if energy:
            for key in result['percent'].keys():
                result['percent'][key] = \
                    result[
                        'total'][key] * ENERGY_FACTOR[key][unit] / energy * 100
    ...
    def clean(self):
        ...
        if self.energy:
            energy_upper = self.energy * (1 + (
                self.ENERGY_APPROXIMATION / Decimal(100.0)))
            energy_lower = self.energy * (1 - (
                self.ENERGY_APPROXIMATION / Decimal(100.0)))
...
class IngredientWeightUnit(models.Model):
    ...
    unit = models.ForeignKey(
        WeightUnit, verbose_name=_('Weight unit'), on_delete=models.CASCADE)
...
class MealItem(models.Model):
    ...
    def get_nutritional_values(self, use_metric=True):
        ...
        nutritional_info['carbohydrates'] += self.ingredient\
            .carbohydrates * item_weight / 100

        if self.ingredient.carbohydrates_sugar:
            nutritional_info[
                'carbohydrates_sugar'] += self.ingredient.carbohydrates_sugar \
                * item_weight / 100

        nutritional_info['fat'] += self.ingredient.fat * item_weight / 100

        if self.ingredient.fat_saturated:
            nutritional_info['fat_saturated'] += self.ingredient\
                .fat_saturated * item_weight / 100

        if self.ingredient.fibres:
            nutritional_info['fibres'] += self.ingredient\
                .fibres * item_weight / 100

        if self.ingredient.sodium:
            nutritional_info['sodium'] += self.ingredient\
                .sodium * item_weight / 100
