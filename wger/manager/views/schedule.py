...
def export_pdf_log(request, pk,
                   images=False, comments=False, uidb64=None, token=None):
    '''
    Show the workout schedule
    '''
    user = request.user
    ...
    # Set the title
    p = Paragraph(u'<para align="center">{0}</para>'.format(
        schedule), styleSheet["HeaderBold"])
    elements.append(p)
    ...
    for step in schedule.schedulestep_set.all():
        ...
        for day in step.workout.canonical_representation['day_list']:
            elements.append(
                render_workout_day(
                    day,
                    images=images, comments=comments, nr_of_weeks=7))
            elements.append(Spacer(10 * cm, 0.5 * cm))
...
def export_pdf_table(request, pk,
                     images=False, comments=False, uidb64=None, token=None):
    '''
    Show the workout schedule
    '''
...
def start(request, pk):
    ...
    return HttpResponseRedirect(reverse(
        'manager:schedule:view', kwargs={'pk': schedule.id}))
